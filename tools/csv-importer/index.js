...
fs.createReadStream(csvPath)
  ...
  .on('end', async () => {
    ...
    const wordsToInsert = results.map((row) => {
    ...
    const statements = wordsToInsert.map(row => {
        return {
            ...
            args: [row.word, row.definition, row.example, row.flagged, row.total_views, row.memorized_count, row.fuzzy_count, row.unknown_count, row.selection_weight]
        };
    });
    ...
    if (outJsonPath) {
        ...
        fs.writeFileSync(outJsonPath, JSON.stringify(jsonWords, null, 2));
        console.log(`\u2714 Successfully generated JSON seed data with ${jsonWords.length} words at: ${outJsonPath}`);
    } else {
